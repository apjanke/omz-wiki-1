...
The only required one is `$ZSH`. The rest either have defaults, or undef is fine and has an
...
Some themes may define additional functions, and set `precmd` or `chpwd` hooks. There's no defined
